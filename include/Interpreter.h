...
// ParsedUnit struct - for reading in generated parser, lexer from the main.cpp entry compilation unit
struct ParsedUnit {
	std::string source;
...
class Interpreter : public TSharpBaseVisitor {
  public:
	...
	// load a program, and run the main function
	void load(TSharpParser::ProgramContext* program);
	void run_main();

	// Visitor method implementations - from ANTLR generated visitor
	// These are all the visitor methods generated from the ANTLR generated visitor C++ class that T# uses
	...
	// Does a value require an assignable?
	void require_assignable(const Value& value, const std::string& declared_type, const std::string& where);
  ...
  private:
	...
	// Contains global environment values, environment values, classes, functions and the current execution result
	...
	// Parsed units - for reading in lexer, parser, etc.
	...
	void eval_arguments(TSharpParser::ArgumentListContext* ctx, std::vector<Value>& out_args);
