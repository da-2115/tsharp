...
program: (NEWLINE | importDecl | topLevelDecl)* EOF;
...
globalVarDecl: modifiers? variableDecl NEWLINE+;
...
enumDecl:
	modifiers? ENUM IDENTIFIER NEWLINE* LBRACE NEWLINE* enumMember (
		COMMA? NEWLINE* enumMember
	)* COMMA? NEWLINE* RBRACE NEWLINE*;

enumMember: IDENTIFIER (ASSIGN INTEGER_LITERAL)?;

inheritanceClause: COLON typeRef (COMMA typeRef)*;

genericParams: LT IDENTIFIER (COMMA IDENTIFIER)* GT;

typeArgs: LT typeRef (COMMA typeRef)* GT;
...
classBody: LBRACE (NEWLINE | classMember)* RBRACE;

interfaceBody: LBRACE (NEWLINE | interfaceMember)* RBRACE;
...
fieldDecl:
	modifiers? typeRef IDENTIFIER arrayDeclarator? (
		ASSIGN expression
	)?;
...
interfacePropertySignature: accessModifier? typeRef IDENTIFIER;

interfaceFieldSignature: accessModifier? typeRef IDENTIFIER;
...
parameter: typeRef IDENTIFIER arrayDeclarator?;

block: NEWLINE* LBRACE NEWLINE* statement* RBRACE;
...
variableStatement: modifiers? variableDecl NEWLINE+;
...
arrayDeclarator: LBRACK expression? RBRACK;

assignmentStatement: assignment NEWLINE+;

assignment: lvalue assignmentOperator expression;

lvalue: primaryLValue lvalueSuffix*;

primaryLValue: IDENTIFIER | THIS | BASE;

lvalueSuffix: DOT IDENTIFIER | LBRACK expression RBRACK;
...
tryStatement: TRY NEWLINE* block catchClause* finallyClause?;
...
throwStatement: THROW expression NEWLINE+;

expression: logicalOrExpression;

logicalOrExpression:
	logicalAndExpression (
		NEWLINE* OR_OR NEWLINE* logicalAndExpression
	)*;

logicalAndExpression:
	equalityExpression (
		NEWLINE* AND_AND NEWLINE* equalityExpression
	)*;

equalityExpression:
	relationalExpression (
		NEWLINE* (EQ | NEQ) NEWLINE* relationalExpression
	)*;

relationalExpression:
	additiveExpression (
		NEWLINE* (LT | LTE | GT | GTE) NEWLINE* additiveExpression
	)*;

additiveExpression:
	multiplicativeExpression (
		NEWLINE* (ADD | SUB) NEWLINE* multiplicativeExpression
	)*;

multiplicativeExpression:
	unaryExpression (
		NEWLINE* (MUL | DIV | MOD) NEWLINE* unaryExpression
	)*;
unaryExpression: (NOT | SUB | ADD | INC | DEC) unaryExpression
	| postfixExpression;

postfixExpression: primary postfixSuffix*;
